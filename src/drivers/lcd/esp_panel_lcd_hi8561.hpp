...
/**
 * @brief LCD driver class for HI8561 display controller
 *
 */
class LCD_HI8561: public LCD {
public:
    /**
     * @brief Default basic attributes for HI8561 LCD controller
     */
     ...
     */
    LCD_HI8561(Bus *bus, int width, int height, int color_bits, int rst_io):
     ...
     * @note Vendor initialization commands may vary between manufacturers. Consult LCD supplier for them
     */
    LCD_HI8561(const BusFactory::Config &bus_config, const Config &lcd_config):
     ...
     */
    [[deprecated("Use other constructor instead")]]
    LCD_HI8561(Bus *bus, uint8_t color_bits = 0, int rst_io = -1): LCD_HI8561(bus, 0, 0, color_bits, rst_io)
    {
    }
...
/**
 * @brief Alias for backward compatibility
 * @deprecated Use `esp_panel::drivers::LCD_HI8561` instead
 */
using ESP_PanelLcd_HI8561 [[deprecated("Use `esp_panel::drivers::LCD_HI8561` instead")]] =
    esp_panel::drivers::LCD_HI8561;
